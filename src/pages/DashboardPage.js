    ...
    //             headers: {
      ...
      <div className="navbar sticky top-0 bg-glass flex items-center justify-between px-4 py-3 md:px-6 md:py-4">
        ...
        <div className="info flex gap-3 md:gap-4 items-center">
          ...
          <div className={`${toggleMenu ? '': 'hidden'} bg-glass rounded py-8 px-7 absolute top-[100px] right-4 bd`}><button className={`text-sm mt-3 rounded bg-teal-600 py-3 px-5 text-white `}>Logout</button></div>        
        </div>
      </div>
      ...
      <div className={` ${showSurvey ? 'fixed top-0 w-screen h-screen bg-white' : 'hidden'}`}>
        <QuestionProvider>
      <SurveyPage/>
