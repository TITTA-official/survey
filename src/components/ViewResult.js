...
import {ResultShowContext, ScoreContext,  QuestionContext, AuthContext} from '../context'
...
function ViewResult() {
  ...
  return (
    <div className={`text-[#000] px-4`}>
      ...
      <div className="heding font-medium">Results for <span className='font-bold'>{user.username}</span></div>
      ...
      <div className="info bg-teal-600 text-white max-w-screen-md py-6 px-8 mx-auto mt-14 rounded text-center ">
        ...
        <div className="result-info font-bold text-base mt-3">{(score/questions.length) * 100}% Secure</div>
      </div>
    </div>
